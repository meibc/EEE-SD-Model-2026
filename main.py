# main.py
"""Main entry point for SEM + CDC pipeline."""
...
from engine.loaders import save, load, load_fit_results
from engine.export import export_unified_table
...
def main(options: MainOptions | None = None) -> dict:
    """Run full pipeline."""
    if options is None:
        options = MainOptions()
    ...
    model_years = build_model_years(
        cdc_loader.years,
        options.target_end_year,
    )
    ...
    sem_loader = None
    state_codes = options.state_intervention_codes if options.enable_interventions else []
    rel_codes = options.relationship_intervention_codes if options.enable_interventions else []

    if options.run_joint_deterministic:
        has_interventions = bool(state_codes or rel_codes)
        if options.run_baseline_comparison and options.enable_interventions and has_interventions:
            joint_baseline = run_joint(
                output, cdc_loader, units,
    ...
    if options.run_joint_uncertainty:
        sem_loader = SEMParamsLoader(options.sem_params_path)
        if options.run_baseline_comparison:
            uncertainty_baseline = run_uncertainty(
                sem_loader, cdc_loader, units,
                unit_ids=all_unit_ids,
            ...
            uncertainty = uncertainty_intervention if options.enable_interventions else uncertainty_baseline
            print(
                f"Saved baseline and intervention uncertainty outputs for {len(uncertainty)} units"
            )
        else:
...
def _show_plots(
    ...
    """Display plots (helper to keep main() clean)."""
    if uncertainty_baseline is not None and uncertainty_intervention is not None:
        ...
        print("Skipping plots: enable run_joint_deterministic or run_joint_uncertainty")
