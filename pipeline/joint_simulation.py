...
class JointRunner:
    ...
    def predict(self, unit_id: str) -> JointResult:
        ...
        tau, prep_on, n_elig, risk_behavior = build_cdc_inputs_from_sem(
            sem_traj=sem_traj,
            unit=self.units[unit_id],
        ...
        if unit_id == "NY":
            print(f"[NY] years: {self.model_years}")
            print(f"[NY] prep_used (aligned): {prep_on}")
            print(f"[NY] risk_behavior (aligned): {risk_behavior}")
            print(f"[NY] N_elig (aligned): {n_elig}")
